...
ipx = IPX()
# 相对与项目根目录的路径
ipx.load(os.path.join(settings.BASE_DIR, "utils/ip_location.datx"))
...
def find_ip_location(ip):
    ...
    :return:  国家，省 ，市，运营商
    """
    ip_info = ipx.find(ip).split('\t')
    return ip_info[0], ip_info[1], ip_info[2], ip_info[4]


def chk_ipaddr(ipaddr):
    ip_pattern = '^((0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5])\.){3}(0|[1-9]\d?|[0-1]\d{2}|2[0-4]\d|25[0-5])$'
    if not ipaddr:
        return False

    ip_check = re.compile(ip_pattern, re.I)
    return True if ip_check.match(ipaddr) else False
...
def mask2cidr(prefix):
    ...
    if not is_valid_mask(prefix):
        return ""
    return sum(bin(int(i)).count('1') for i in prefix.split('.'))
...
def random_china_ip(num=1):
    """随机选择中国IP
    """
    ips = []
    for i in range(num):
        alls = IPNetwork(random.choice(chinaip))
        ip_addr = random.choice(list(alls)).format()
        try:
            ip_location = find_ip_location(ip_addr)[1]
        except Exception:
            ip_location = "未知"
        
        ips.append([ip_addr, ip_location])
    return ips
