...
class vulscan():
    ...
    def start(self):
        ...
        if '.json' in self.plugin_info['filename']:  # 标示符检测模式
            try:
                ...
                logger.error('启动扫描任务出错!'.format(e.message))
                return
        else:  # 脚本检测模式
            plugin_filename = self.plugin_info['filename']
            logger.info(str(self.task_netloc) + "call " + self.task_plugin)
            if self.task_plugin not in PLUGIN_DB:
    ...
    def load_json_plugin(self):
        path = os.path.join('soc_scan', 'tools', 'vulscan', 'vuldb')
        json_plugin = open(path + '/' + self.plugin_info['filename']).read()
    ...
    def save_request(self):
        if self.result_info:
            try:
                ...
                logger.info(str(self.task_netloc) + " " + self.result_info)
...
def init():
    ...
    for plugin_name in json_plugin:
        try:
            path = os.path.join('soc_scan', 'tools', 'vulscan', 'vuldb')
            json_text = open(path + '/' + plugin_name, 'r').read()
