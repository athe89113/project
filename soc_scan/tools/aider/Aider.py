            ...
            if len(route_list) == 3:
                ...
                for query_raw in query_history:
                    if query_str in query_raw:
                        ...
                        html = "YES"
            logger.info(datetime.datetime.now().strftime('%m-%d %H:%M:%S') + " " + str(addr[0]) + ' web query: ' + path)
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            logger.info(datetime.datetime.now().strftime('%m-%d %H:%M:%S') + " " + str(addr[0]) + ' Dns Query: ' + recv)
        except Exception, e:
            logger.error(e)
            continue
