...
class scan:
    ...
    def run(self):
        ...
        for _port in self.port_list:
            ...
            if self.server == '':
                ...
                if web_info:
                    logger.info('ip:{},port:{},web_info:{}'.format(self.ip, self.port, web_info))
    ...
    def scan_port(self):
        ...
        try:
            ...
            self.banner = 'NULL'
        logger.info('portscan==ip:{},port:{}'.format(self.ip, self.port))
        banner = ''
        ...
        if not self.server and self.port not in [80, 443, 8080]:
            for mark_info in self.config_ini['Discern_server']:  # 发包识别
                try:
                    ...
                    pass
        if self.server:
            logger.info('server==ip:{},port:{},server:{}'.format(self.ip, self.port, self.server))
