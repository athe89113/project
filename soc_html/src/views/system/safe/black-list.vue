    ...
    <div class="tool-box">
      ...
      <div>
        <validator name="valCompany" @valid="onValid = true" @invalid="onValid = false">
            ...
            <div class="ys-box">
              <div class="ys-box-title">添加IP到黑名单</div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      add () {
        this.$http.post('/api/system/system_safe', {'ip': this.addip, 'is_black': 1, 'admin_password': this.adminPassword}).then(function (response) {
          if (response.data.status === 200) {
            this.$root.alertSuccess = true;
            this.$root.errorMsg = response.data.msg
