<template>
    <div class="ys-con pos-r">
        <div v-show="!showTimeLine">
            <div class="ys-con pos-r">
                <div class="ys-box-con">
                    <div>
                        <span class="ys-info-color m-r-5 verticalM">事件来源：</span>
                        <ys-select :option="eventSourceData" :selected.sync="curEventSource"></ys-select>
                        <span class="ys-info-color m-r-5 m-l-10 verticalM">事件类型：</span>
                        <ys-select :option="eventTypeData" :selected.sync="curTypeSource"></ys-select>
                        <span class="ys-info-color m-r-5 verticalM m-l-10">关键词搜索：</span>
                        ...
                        <input class="ys-input m-r-5" placeholder="IP" v-model="search_ip"/>
                    ...
                    <p class="m-t-10">综合条件查询<a class="m-l-10" @click="filterStatus=!filterStatus"><span
                    ...
                    <div class="filter-wrap">
		                ...
		                <span v-for="single in list">
		                  <span class="ys-success-color m-r-5 m-l-5 font14 verticalM" v-show="single.logic==1">且</span>
		                  ...
		                  <span class="verticalM" v-if="single.field.id=='event_level'">[ {{single.value | levelFil}} ]</span>
                           <span class="verticalM" v-if="single.field.id=='event_two_type'">[ {{single.value | eventTwoTypeFil}} ]</span>
                           <span class="verticalM" v-if="single.field.id=='opt_result'">[ {{single.value | optResultFil}} ]</span>
                            <span class="verticalM" v-if="single.field.id=='app_protocol'">[ {{single.value | appProtocolFil}} ]</span>
                            <span class="verticalM" v-else>[ {{single.value}} ]</span>
		                  <span class="verticalM ys-error-color text-cursor" @click="deleteCondition(single,list)"><i
                            ...
                            <div>
                                ...
                                <ys-select :option="eventTwoTypeData" :selected.sync="curEventTwoType"
                                           v-if="addCondition.field.id=='event_two_type'"></ys-select>
                                <ys-select :option="levelData" :selected.sync="curLevel"
                                           v-if="addCondition.field.id=='event_level'"></ys-select>
                                <ys-select :option="optResultData" :selected.sync="curOptResult"
                                           v-if="addCondition.field.id=='opt_result'"></ys-select>
                                <ys-select :option="appProtocolData" :selected.sync="curAppProtocol"
                                           v-if="addCondition.field.id=='app_protocol'"></ys-select>
                                <input class="ys-input" v-model="addCondition.value"
                                       v-if="addCondition.field.id!='opt_result'&&addCondition.field.id!='event_level'&&addCondition.field.id!='event_two_type'&&addCondition.field.id!='app_protocol'"/>
                ...
                <div class="m-t-10">
                    <p class="split-line">
                        <button class="ys-btn" @click="goSearch(false)"><i class="ys-icon icon-search"></i>查询</button>
                    </p>
                    <div class="ys-box-con">
                        <div class="m-b-10">
                            <div class="ys-box-con box-title">
                                <div class="clearfix">
                                    <span class="ys-info-color fLeft m-r-10" style="padding-top:4px;">图表分析</span>
                            ...
                            <div v-if="lineShow" class="ys-box-con">
                                <div class="p-t-10 ys-box-con p-b-10">
                                    <div class="clearfix">
                        ...
                        <div class="custom-table m-t-10">
                            <table class="ys-table">
                                ...
                                <tr>
                                    <th></th>
                                    <th v-for="head in resultHeader" track-by="$index" :style="{'width':head.name == '事件内容'?'340px':'auto'}">{{head.name}}</th>
                                    <th v-if="resultHeader.length > 0">操作</th>
                                </tr>
                                </thead>
                                ...
                                <tr>
                                    ...
                                    <td v-for="item in resultHeader"
                                        ...
                                        style="width:300px;" :title="tdFilterData(item, list)">
                                        {{tdFilterData(item, list)}}
                                    </td>
                                    <td class="text-cursor origins-note" @click="goOriginsNote(list)">
                                        溯源日志
                                    </td>
                                </tr>
                                <tr class="detail" v-bind:class="[curDetailId == $index ? 'open' : '' ]">
                                    <td :colspan="resultHeader.length + 2">
        ...
        <div v-show="showTimeLine">
            <event-timeline :ip="passInfo.ip" :pass-tag="passInfo.tag" :pass-time="passInfo.time"
                            :show.sync="showTimeLine"></event-timeline>
        </div>
    </div>
</template>
<style scoped>
    ...
    export default {
        ...
        data() {
            return {
                ...
                tableUrl: "/api/ssa/event/analysis_search/dts",
                tableList: "",
                tableTotal: 0,
                tableFilter: {
                    data_tag: {id: 1},
        ...
        ready() {
            ...
            Promise.all([this.getEventSourceData()]).then((result) => {
                Promise.all([this.getEventTypeData()]).then((result) => {
        ...
        filters: {
            ...
            eventTwoTypeFil: function (id) {
                for (let x in this.eventTwoTypeData) {
                    if (id == this.eventTwoTypeData[x].id) {
                        return this.eventTwoTypeData[x].name
                    }
                }
            },
            optResultFil: function (id) {
                for (let x in this.optResultData) {
                    if (id == this.optResultData[x].id) {
                        return this.optResultData[x].name
                    }
                }
            },
            appProtocolFil: function (id) {
                for (let x in this.appProtocolData) {
                    if (id == this.appProtocolData[x].id) {
                        return this.appProtocolData[x].name
                    }
                }
            },
        ...
        methods: {
            ...
            showXConfig() {
                this.$http.post('/api/ssa/event/analysis_select_search/list', {tag: this.curEventSource.id}).then(function (response) {
            ...
            getEventSourceData() {
                return new Promise((resolve) => {
                    this.$http.post('/api/ssa/event/analysis_tags/list').then(function (response) {
                        this.eventSourceData = [{id: 0, name: '全部'}].concat(response.data.items);
            ...
            getEventTypeData() {
                return new Promise((resolve) => {
                    this.$http.post('/api/ssa/event/analysis_type_tags/list', {tag: this.curEventSource.id}).then(function (response) {
                        this.eventTypeData = [];
                        ...
                        for (let key in data) {
                            ...
                            this.eventTypeData.push(obj);
                        }
                        this.eventTypeData = [{id: 0, name: '全部'}].concat(this.eventTypeData);
                        this.curTypeSource = this.eventTypeData[0];
                        resolve();
                    }, function (response) {
                        Api.user.requestFalse(response, this);
                    })
            ...
            getFieldData() {
                this.$http.post('/api/ssa/event/analysis_select/list', {tag: this.curEventSource.id}).then(function (response) {
                    try {
                ...
                this.$http.post('/api/ssa/event/analysis_select_search/list', {tag: this.curEventSource.id}).then(function (response) {
                    ...
                    for (let key in response.data.items) {
                        ...
                        if (item.indexOf('时间') > -1 || item.indexOf('事件一级类') > -1 || item.indexOf('事件来源') > -1 || item.indexOf('事件二级类') > -1) {
                            continue;
                        } else {
            ...
            getFieldMapData() {
                this.$http.get('/api/ssa/event/analysis_field_map', {tag: this.curEventSource.id}).then(function (response) {
                    this.fieldMapData = response.data.data;
                    if (this.fieldMapData.event_level) {
                        this.levelData = this.fieldMapData.event_level.items;
                        this.curLevel = this.levelData[0];
                    }
                    if (this.fieldMapData.event_two_type) {
                        this.eventTwoTypeData = this.fieldMapData.event_two_type.items;
                        this.curEventTwoType = this.eventTwoTypeData[0];
                    }
                    if (this.fieldMapData.opt_result) {
                        this.optResultData = this.fieldMapData.opt_result.items;
                        this.curOptResult = this.optResultData[0];
                    }
                    if (this.fieldMapData.app_protocol) {
                        this.appProtocolData = this.fieldMapData.app_protocol.items;
                        this.curAppProtocol = this.appProtocolData[0];
                    }
                }, function (response) {
                    Api.user.requestFalse(response, this);
                })
            ...
            getFieldPercentData() {
                let data = {
                    event_source: this.curEventSource.id,
                    data_tag: 1,
                ...
                data.query = arr;
                this.$http.post('/api/ssa/event/analysis_search/value_percent', data).then(function (response) {
            ...
            tableRe() {
                this.tableFilter.data_tag.id = 1;
                this.tableFilter.event_source.id = this.curEventSource.id;
                this.tableFilter.event_one_type.id = this.tablePostData.event_one_type;
                ...
                this.tableFilter.search_ip.id = this.tablePostData.search_ip;
            ...
            addSetting() {
                if (!this.addCondition.value && this.addCondition.field.id != "event_level" && this.addCondition.field.id != "event_two_type" && this.addCondition.field.id != "opt_result" && this.addCondition.field.id != "app_protocol") {
                    this.$root.alertError = true;
                    this.$root.errorMsg = '筛选值不能为空';
                    return false;
                }
                if (this.addCondition.field.id == "event_level" && !this.curLevel.id) {
                    this.$root.alertError = true;
                    this.$root.errorMsg = '筛选值不能为空';
                    return false;
                }
                if (this.addCondition.field.id == "event_level") {
                    this.addCondition.value = this.curLevel.id;
                }
                if (this.addCondition.field.id == "event_two_type" && !this.curEventTwoType.id) {
                    this.$root.alertError = true;
                    this.$root.errorMsg = '筛选值不能为空';
                    return false;
                }
                if (this.addCondition.field.id == "event_two_type") {
                    this.addCondition.value = this.curEventTwoType.id;
                }
                if (this.addCondition.field.id == "opt_result") {
                    this.addCondition.value = this.curOptResult.id;
                }
                if (this.addCondition.field.id == "app_protocol" && !this.curAppProtocol.id) {
                    this.$root.alertError = true;
                    this.$root.errorMsg = '筛选值不能为空';
                    return false;
                }
                if (this.addCondition.field.id == "app_protocol") {
                    this.addCondition.value = this.curAppProtocol.id;
                }
            ...
            goSearch(status) {
                ...
                let data = {
                    event_source: this.curEventSource.id,
                    data_tag: 1,
                    event_one_type: this.curTypeSource.id,
                    start_time: this.startTime,
                    end_time: this.endTime,
                    query_string: this.keyword,
                    search_ip: this.search_ip,
                ...
                this.searchParams.tag = this.curEventSource.id;
                let arr = [];
                for (let parent in this.conditionList) {
                    let arr1 = [];
                    for (let child in this.conditionList[parent]) {
            ...
            getChartData() {
                ...
                data.x = xList;
                data.y = yList;
                this.$http.post('/api/ssa/event/analysis_search/line', data).then(function (response) {
                    this.$root.loadStatus = false;
                    if (response.data.status == 200) {
                ...
                let lineOption = {
                    ...
                    tooltip: {
                        ...
                        formatter: function (params) {
                            ...
                            for (let x in params) {
                                if (params[x].seriesName.indexOf("流量") != -1) {
                    ...
                    yAxis: [{
                        ...
                        axisLabel: {
                            textStyle: {color: "#dfdfe7"},
                            formatter(params) {
                                if (JSON.stringify(self.eventDetectData.name).indexOf('流量') != -1) {
            ...
            goOriginsNote(list) {
                ...
                this.passInfo.ip = list.src_ip;
                this.passInfo.tag = 1;
                this.passInfo.time = list['end_time'];
                this.showTimeLine = true;
                //window.open('event-timeline?ip='+list.src_ip+'&tag='+this.searchParams.tag+'&time='+list['time'], '_blank');
            }
        },
        watch: {
