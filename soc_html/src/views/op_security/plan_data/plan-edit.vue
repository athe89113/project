    ...
    <div class="pos-r m-t-10">
      <div class="box-left" style="margin-right:223px;">
        <div class="ys-box m-t-5">
          <div class="ys-box-title">修改预案</div>
          <div class="ys-box-con">
            <table class="bigTable">
              ...
              <tr>
                <td>TAG标签:</td>
                <td colspan="3">
                  <tagator :width="220" :list.sync="tagInput"></tagator><!--<span>*必选项</span>-->
                </td>
              </tr>
              <tr>
                <td>预案简介:</td>
                <td colspan="3">
                  <textarea class="ys-textarea" placeholder="1234" v-model="submitData.brief"></textarea>
                </td>
              </tr>
              <tr>
                <td>详细内容:</td>
            ...
            <div class="m-t-10 m-b-10">
              ...
              <button class="ys-btn ys-btn-white m-r-10" @click="href('plan-mgt')">取消</button>
            </div>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    data () {
      return {
        ...
        "editPort":"/api/op_store/plan/detail/",
        tableFilter:{"id":""},
        "recentAdd":[{
          "title":"...",
          "level":"...",
    ...
    methods:{
      ...
      initDetail:function(){
        this.$http.get(this.detailPort+this.$route.params.id,{}).then(function(response){
          if(Number(response.data.status)===200){
            ...
            this.submitData={
              title:response.data.data.title,// 名称 
              user:response.data.data.user,// 添加人 
              level:response.data.data.level,//级别，
              scene:response.data.data.scene,//应用场景
              tag:response.data.data.tag.join(","),//标签 逗号分割
              brief:response.data.data.brief,//简介
              description:response.data.data.description,//内容
              contact:""//response.data.data.contact,//管理员用户id 逗号分割
            };
            this.contentHtml=this.submitData.description;
            this.currentLevelSelect={
              "id":this.submitData.level,
              name:this.submitData.level+"级"
            };
            this.currentSceneSelect={
              "id":this.submitData.scene,
              name:this.submitData.scene
            };
            this.tagInput=response.data.data.tag;
            this.contactData=[];
            for(let i=0; i<response.data.data.contact.length; i++){
              if(response.data.data.contact[i]["name"]!=="--") this.contactData.push(response.data.data.contact[i]);
            }
      ...
      initPage:function(){
        ...
        this.submitData={
          ...
          brief:"",//简介
          description:'',//内容
          contact:'',//"liubo&1368888888&liubo@qq.com,",多条数据逗号分割
        };
      ...
      submitPage:function(){
        ...
        if(errsLength>0){
          ...
          let self=this;
          this.$http.put(this.editPort+this.$route.params.id,this.submitData).then(function(response){
            if(Number(response.data.status)===200){
